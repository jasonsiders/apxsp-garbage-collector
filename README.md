...
Introducing `apxsp-garbage-collector` - a metadata driven solution to data storage problems. This framework provides a way to quickly create batch jobs to handle stale data. Simple jobs - like the example above, can be created or modified in metadata, without touching a single line of code. The framework can be easily extended to handle more complex use cases.

## Creating a Scheduled Job

Each job (or instance of the `apxsp.GarbageCollector` base class) is designed to run inside of a scheduled job. Developers can create regular instances of `apxsp.GarbageScheduler` in the [Setup UI](https://help.salesforce.com/s/articleView?id=sf.code_schedule_batch_apex.htm&type=5). For finer control, Developers can call `System.schedule()` in an anonymous script, like the one below:

```
// Creates a job to run daily at midnight
final String CRON_EXP = '0 0 0 * * ?';
final String jobName = 'Garbage Collection';
System.schedule(jobName, CRON_EXP, new apxsp.GarbageScheduler());
```
...
The `apxsp__Garbage_Collection_Job__mdt` custom metadata object gives developers declarative control over the framework. Each record represents a single job that will be run during each run of `apxsp.GarbageScheduler`.

**Fields**:

-   `Active`: Controls whether the job will be run in `apxsp.GarbageScheduler`.
-   `Priority`: A number that determines the order that jobs are run in.
-   `Batch Size`: Determines batch size of the resulting `apxsp.GarbageCollector` batch job. This defaults to 200. You may increase the size to reduce the number of batches (for extremely large jobs), or decrease it to avoid CPU timeouts.
-   `SObjectType`: The `FROM` SObject in the batch query.
-   `Apex Class Name`: The name of the custom Apex Class to be run. One of this field, or `SOQL Filter` **must** be populated. This class must extend the `apxsp.GarbageCollector` base class. See [Custom Jobs](#CustomJobs) for more.
...
For each of these _SOQL-driven jobs_, `apxsp.GarbageScheduler` will create a new instance of the `GarbageCollectorSoql` class. This class will construct a batch query using the `Garbage Collection Job` record's `SObjectType` and `SOQL Filter` values, and then delete any records that the query returns.
...
-   Extend the `apxsp.GarbageCollector` abstract class
-   Ensure the class can be constructed with a zero-arg constructor, ex., `new MyCustomDeleteJob()`
...
Some organizations may use a third-party service to archive data, instead of deleting it altogether. Developers can accomplish this by creating a new job which overrides `apxsp.GarbageCollector`'s virtual `execute()` method:

```
public class ArchiveExample extends apxsp.GarbageCollector {
...
A _SOQL-driven job_ is limited to basic SOQL queries. The query cannot utilize any advanced Apex features, such as bind variables.
...
```
public class AdvancedSoqlExample extends apxsp.GarbageCollector {
